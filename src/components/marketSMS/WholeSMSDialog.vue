        ...
        <div slot="fileUpdate" v-if="formData.value === 0">
          ...
          <div style="margin-top: 6px;margin-left: 15px;">
            <p v-for="(item, index) in fileName" :key="index" style="color: #20232a;">
              ...
              <el-button v-if="item.errormsgs.length > 0" @click="detailsUpdateFiles(index)" type="text" style="margin-left: 20px;color: #0887e0;">失败详情</el-button>
            </p>
          </div>
        </div>
      </m-form>
    ...
    const vnode = (
      <div style="border-radius: 10px;">
        ...
        <p style="text-align:center;margin-bottom: 17px;">
          <span style="color: #333333;font-size: 16px;font-weight: 500;">成功导入</span> <span style="color: #0887e0;font-weight: 700;font-size: 16px;">{ data.count - data.fail }</span>&nbsp;&nbsp;
