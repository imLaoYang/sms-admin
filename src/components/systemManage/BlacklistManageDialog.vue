        ...
        <div slot="fileList">
          <div style="margin-top: 6px;margin-left: 50px;">
            <p v-for="(item, index) in fileName" :key="index" style="color: #20232a;">
              ...
              <el-button v-if="fileName[index].isShow" @click="detailsUpdateFiles(index)" type="text" style="margin-left: 20px;color: #0887e0;">失败详情</el-button>
            </p>
          </div>
        </div>
      </m-form>
    ...
    const vnode = (
      <div style="border-radius: 10px;top:18%">
        ...
        <p style="text-align:center;margin-bottom: 17px;">
          <span style="color: #333333;font-size: 16px;font-weight: 500;">成功导入</span> <span style="color: #0887e0;font-weight: 700;font-size: 16px;">{ data.total - data.fail }</span>&nbsp;&nbsp;
